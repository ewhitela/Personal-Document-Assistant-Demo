...
class FakeRAG:
    ...
    def answer(self, question):
        ...
        src = [
            Passage(
                "The refund window is 30 days.", "refunds.txt", 0.9, "refunds.txt::0"
            )
        ]
        return RAGAnswer(
            answer=self.llm.generate("prompt"), sources=src, prompt="prompt"
        )
...
def test_answer_text_routes_to_rag():
    ...
    assert bot.rag.last_question == "how long for a refund?", (
        "should pass the question to rag.answer"
    )
...
def test_answer_spoken_chains_stt_rag_tts():
    ...
    assert q == "what is the refund window?", (
        "transcript should come from the transcriber"
    )
    assert isinstance(res, RAGAnswer) and "30 days" in res.answer, (
        "rag should answer the transcript"
    )
...
def test_ready_aggregates_modules():
    ...
    status = bot.ready()
    assert (
        status.get("stt") is True
        and status.get("llm") is True
        and status.get("tts") is True
    )
    assert status.get("vision") is True, (
        "vision status should be present when a vision module is set"
    )
