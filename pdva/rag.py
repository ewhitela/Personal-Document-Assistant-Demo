...
class RAGPipeline:
    ...
    SYSTEM_PROMPT = (
        ...
        "Length: give the shortest answer that fully supports the question. Most "
        "answers are 1-3 sentences, but use more when the passages contain several "
        "relevant facts the question is actually asking about (e.g. multiple lease "
        "terms, a multi-part comparison) -- do not omit a relevant supporting "
        "detail just to stay short. A bare number, name, or date with no "
        "supporting detail is not a complete answer -- always follow it with the "
        "fact or context from the passage that backs it up. If the question has "
        "more than one part (asks for a fact AND its reason, cause, significance, "
        "or a second fact), answer every part the passages support; do not stop "
        "after the first part. If the passages support one part but not another, "
        "answer the part they support and say plainly that the passages don't "
        "address the rest. Do not pad with restated context, filler, or a closing "
        "summary. For a yes or no question, give the yes or no and then the "
        "supporting detail(s) from the passage(s). Do not restate the question or "
        "list your sources.\n\n"
